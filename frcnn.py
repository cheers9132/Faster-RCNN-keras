...
class FRCNN(object):
    ...
    # 该方法通过在字典"_defaults"中查询给定的参数n
    # 如果找到了该参数，则返回该参数的值；如果没有找到该参数，则返回一个错误信息，告诉用户该参数名称不存在。
    @classmethod
    ...
    def generate(self):
        ...
        # 否则先构建模型再载入
        # frcnn.get_predict_model函数，该函数获取预测模型并返回两个模型，一个是RPN模型，一个是分类器模型。
        ...
        # 画框设置不同的颜色，使用 hsv 色彩空间确定颜色，并将 hsv 色彩空间转换为 rgb 色彩空间，以方便后续使用。
    ...
    """
    这段代码实现了计算图像最终输出长和宽。
    它定义了一个内部函数 get_output_length(input_length)，用于计算单边（宽或高）的输出长度。
    该内部函数使用卷积、填充和步幅来计算输出长度。使用四次卷积操作，每次卷积使用不同的卷积核大小和
    填充大小，以此缩小输入图像的大小。最后，通过除以步幅并加 1 计算输出长度。
    最终，函数 get_img_output_length(self, width, height) 调用内部函数并返回两次调用的结果，
    分别为最终的输出宽和高。"""
    ...
    def detect_image(self, image):
        ...
        # 将预测结果进行解码
        # 接下来，该算法对预测结果进行解码，得到候选区域的坐标信息。然后对候选区域进行筛选，
        # 去掉面积小于1的区域。将保留下来的候选区域输入到一个分类器模型中进行分类，得到每个区域属于各个类别的概率值。
